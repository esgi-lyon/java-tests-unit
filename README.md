...
- **La vue** : On créer avec des modules Swing des composants graphiques et des formulaires. La vue appelle un musicalEvent Layout central conservant la `mainFrame`  soit la fenêtre visuelle correspondant à notre logiciel.
...
- **Le model** : correspond aux objets représentants les entités de notre projet comme le `Car`, ou le `Client`. Ils sont accessibles au travers du musicalEvent EntityManager . Ce musicalEvent permet de gérer les opérations CRUD dans la base de données sur une entité spécifique.

## Registre de musicalEvent

Dans ce pseudo framework tout les objets qui transite dans les différents Contrôleurs sont des **services** et implémentent l'interface `Framework.Service` . Un musicalEvent non `load` déclenche une erreur qui peut être fatale pour le fonctionnement de l'application.

Voici les méthodes obligatoires d'un musicalEvent si jamais vous avez besoin de créer le votre...
```java
    @Override
    public void load() {
        loaded = true;
...
- `this.getService()` pour récupérer un musicalEvent, attention si vous voulez récupérer une classe particulière il faut bien casté l'instance Framework.Service vers celle de votre classe avec par exemple: 
```java
...
Ce musicalEvent permet la récupération et la sauvegarde sur la base de données.
...
#### Le cas du musicalEvent Layout : 

Le musicalEvent Layout créer avec les composants swing nous permet de gérer la pageBtn d'accueil et donc de créer des liens vers les pages.
...
Voici les propriété du musicalEvent layout : 
```java
